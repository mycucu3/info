...
配置后进入 `Actions -> Daily AI Coding News -> Run workflow` 手动测试一次。测试通过后，会每天北京时间 11:15 自动推送到微信。
...
GitHub Actions 使用 UTC 时间，这表示北京时间每天 11:15。避开整点可以降低 GitHub 定时任务延迟或被丢弃的概率。
